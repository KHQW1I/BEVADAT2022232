...
def csv_to_df(file_path):
    try:
        ...
        print(f"{file_path} fájl nem található!")
...
def capitalize_columns(df):
    ...
    for col_name in df.columns:
        if 'e' not in col_name:
            renamed_cols[col_name] = col_name.upper()
...
def average_scores(df):
    avg_scores = df.groupby('parental level of education').mean()[['math score', 'reading score', 'writing score']]
    return avg_scores
...
def add_age(df):
    random.seed(42)
    df['age'] = [random.randint(18, 66) for _ in range(len(df))]
    return df
...
def female_top_score(df):
    ...
    max_math_score = np.max(female_scores['math score'])
    max_reading_score = np.max(female_scores['reading score'])
    max_writing_score = np.max(female_scores['writing score'])
...
def ethnicity_pie_chart(df):
    ethnicity_count = df['race/ethnicity'].value_counts()
    ethnicity_percentages = ethnicity_count / sum(ethnicity_count) * 100

    fig, ax = plt.subplots()
    ax.pie(ethnicity_percentages, labels=ethnicity_percentages.index, autopct='%1.1f%%')
